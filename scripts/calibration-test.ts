 ...
 *
 * Usage: npx ts-node scripts/calibration-test.ts
 *
...
interface TestBusiness {
  ...
  industry: string
  industryKey: string
  expectedTier: 'high' | 'medium' | 'low' | 'unknown'
}
...
async function runTest(business: TestBusiness): Promise<TestResult> {
  try {
    ...
    const localResult = data.results?.find((r: any) => r.prompt?.includes('recommend'))
    const directResult = data.results?.find((r: any) => r.prompt?.includes('tell me about'))
...
async function runAllTests(): Promise<TestResult[]> {
  ...
  for (let i = 0; i < TEST_BUSINESSES.length; i++) {
    ...
    process.stdout.write(`${progress} Testing: ${business.name} (${business.city})... `)
    ...
    if (result.error) {
      ...
      console.log(`${scoreEmoji} Score: ${result.overallScore} (${result.rating})`)
    }
...
function generateStatistics(results: TestResult[]): void {
  ...
  for (const tier of ['high', 'medium', 'low'] as const) {
    ...
    if (tierResults.length > 0) {
      ...
      console.log(`   Expected ${tier.toUpperCase()}: avg ${tierAvg.toFixed(1)} (n=${tierResults.length})`)
    }
  }
  ...
  for (const industry of industries) {
    ...
    console.log(`   ${industry}: avg ${industryAvg.toFixed(1)} (n=${industryResults.length})`)
  }

  // Potential clients (low scores = sales opportunities)
  const lowScoreBusinesses = validResults.filter(r => r.overallScore < 40)
  console.log(`\n🎯 Sales Opportunities (Score < 40): ${lowScoreBusinesses.length} businesses`)
  console.log(`   These businesses need AI visibility optimization!`)
}

function exportToCSV(results: TestResult[], filename: string): void {
  const headers = [
    'Business Name',
    'City',
    'Industry',
